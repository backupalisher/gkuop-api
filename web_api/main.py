    ...
    try:
        ...
        if search:
            where_clauses.append(
                "(t.ticket_number ILIKE %s OR t.subject ILIKE %s OR "
                "t.inventory_number ILIKE %s OR t.author_name ILIKE %s OR "
                "t.printer_model ILIKE %s OR "
                "t.component ILIKE %s OR "
                "t.soglasovano_line ILIKE %s OR "
                "t.work_done ILIKE %s OR "
                "t.tech_conclusion ILIKE %s OR "
                "t.cause ILIKE %s)"
            )
            like_val = f"%{search}%"
